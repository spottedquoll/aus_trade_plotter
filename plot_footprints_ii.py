...
f_nested = h5py.File(filename, 'r')

filename = birds_dir + '/results/' + 'footprints_global.h5'
f_global = h5py.File(filename, 'r')

# Relate the base regions to the SA2 regions
subnat_region_legend = flatten_list(list(f_nested['subnat_region_legend']))
...
assert f_nested[list(f_nested)[0]].shape[1] == len(bird_labels)  # labels dim matches data
assert f_global[list(f_global)[0]].shape[1] == len(bird_labels)
...
filter_birds_list = ['Buff-breasted Button-quail', "Coxen's Fig-Parrot", 'Golden-shouldered Parrot'
                     ...
                     #, 'Gulf St Vincent Slender-billed Thornbill']
...
for field_name in fields_to_plot:
    ...
    if field_name in list(f_nested):
        footprints_by_subregion = list(f_nested[field_name])
    elif field_name in list(f_global):
        footprints_by_subregion = list(f_global[field_name])
    else:
        raise ValueError('Field does not exist in results')
    ...
    if 'global_mrio' in field_name:
        global_total = sum(intensity_by_sa2)
